...
UPLOAD_PDF = 'static/uploads/pdfs'
...
def search():
    ...
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
        cur.execute("SELECT productId, name, price, description, image FROM products WHERE LOWER(name) LIKE ?", ('%' + query + '%',))
        itemData = cur.fetchall()
        cur.execute('SELECT categoryId, name FROM categories')
        categoryData = cur.fetchall()
    itemData = parse(itemData)
    ...
    if request.method == "POST":
        ...
        with sqlite3.connect('database.db') as conn:
            try:
                cur = conn.cursor()
                cur.execute(
                    '''INSERT INTO products (name, price, description, image, categoryId) VALUES (?, ?, ?, ?, ?, ?)''',
...
def productDescription():
    ...
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
        cur.execute(
            'SELECT productId, name, price, description, image FROM products WHERE productId = ' + productId)
...
def download():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        userId = cur.fetchone()[0]
        cur.execute(
            "SELECT products.pdf FROM products, kart WHERE products.productId = kart.productId AND kart.userId = " + str(
                userId))
        products = cur.fetchall()
    files_to_zip = []
    for row in products:
        files_to_zip.append(UPLOAD_PDF + "/" + row[0])
